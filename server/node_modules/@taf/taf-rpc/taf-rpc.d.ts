...
import { DyeingObject } from '@taf/taf-dyeing'
// import { EventEmitter } from 'events'
import { BinBuffer, Map, UniAttribute } from '@taf/taf-stream'
import { Configure, Endpoint } from '@taf/taf-utils'
import { BasePacket } from '@taf/taf-rpc-jce'
...
export class NetworkBuffer {
  ...
  //内部是buffer list, buffer list的长度(buffer的个数)
  getBufferListLength(): number;

  //添加buffer
  addBuffer(data: Buffer): void;
  ...
  //读取一定长队buffer
  getBuffers(length: number): Buffer;

  //读取头四个字节长度
  readLength(): number

  //移动position(读取了完了, 调用该函数移动一下)
  moveHeader(len: number): boolean

  //分配
  static New(): NetworkBuffer
}

export interface JceCurrent {
  ...
  /** 仅 TAF 协议有效. 开发者调用 `sendResponse`, 编码之后, 由框架调用该函数返回数据, 业务不需要自己主动调用 */
...
//   readFromObject(json: BasePacket): void;

//   toBinBuffer(): Buffer;

//   static New(): BasePacket;
// }
...
export interface ProtoMessage {
  //数据buffer, TAF协议存放的就是BasePacket, 如果是第三协议存在的数据包(origin至少有三个字段: {iRet, iRequestId, sBuffer}, iRet=0表示成功, iRequestId: 请求的Id, sBuffer: 存放实际传输的buffer)
  origin: any
  //请求响应包Id, 该字段必填写, 用来映射请求和响应包, 如果是TAF协议, 则从origin中复制iRequestId出来
  iRequestId: number,
  //函数名, 第三方协议, 可以解析出来, 用来调用Imp中的函数
  sFuncName: string,
  //第三方协议支持时使用, 调用rpc函数是, 第二个参数以后的数据都放在appBuffer中, 并在compose中, 业务使用appBuffer打包成最终的Buffer
  appBuffer: any[]
  //  iResultCode: number,
  //  sResultDesc: string
}
...
export interface ObjectProxy {
  ...
  taf_invoke(funcName: string, binBuffer: BinBuffer, property?: InvokeProperty, functionInfo?: SharedFunctionInfo): Promise<RpcResponse>
  wup_invoke(funcName: string, uniAttribute: UniAttribute, property?: InvokeProperty, functionInfo?: SharedFunctionInfo): Promise<RpcResponse>
...
export class Communicator {
  ...
  /**
   * 生成代理类接口
   * @param ProxyHandle 由 jce2node 生成的 Proxy 类
   * @param objName 被调服务名
...
export interface Protocol { //extends EventEmitter {
  ...
  /** 根据传入数据进行打包的方法 */
  compose(params: ProtoMessage): Buffer,

  /**
   * 网络收取包之后, 填入数据判断是否完整包
   * @param data 传入的 `data` 数据可能是NetworkBuffer
   */
  feed(data: NetworkBuffer, message: ProtoMessage): boolean,
...
export interface BindAdapterOptions {
  ...
  /** 当前 BindAdapter 上的协议解析器, 如果不设置或者为 `taf` 则默认为 TAF 协议 */
  protocol?: 'taf' | ProtocolConstructor,
...
export interface HeroServer {
  ...
  /** 设置协议解析器 */
  addServantProtocol(ServantObj: string, protocol: Function): void;
  /** 启动服务 */
  start(options?: BindAdapterOptions): Promise<any[]>
  start(options: ServantConfig): Promise<any[]>
...
export class ServantProxy {
  ...
  setProtocol(protocol: ProtocolConstructor): void
  setPushCallbacks(callback: (message: ProtoMessage) => void): void
...
export { Promise as promise } from '@taf/taf-utils'
