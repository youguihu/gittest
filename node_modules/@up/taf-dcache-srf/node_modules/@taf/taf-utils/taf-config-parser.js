...
Configure.prototype.parseFile = function (sFilePath, encoding, callback) {
    ...
    try {
        ...
        error.code      = -1;
        error.message   = "read file fail";
        error.exception = e;
        console.error("parseFile exception", sFilePath, e);
    }
...
Configure.prototype.getDomain = function (key, DEFAULT_VALUE) {
    ...
    if (domain instanceof Object) {
        for (var key in domain) {
            if (domain[key] instanceof Object && key !== 'taf_config_line_vector_value') {
                maps.push(key);
            }
        }
    }
...
Configure.prototype.getDomainLine = function(key, DEFAULT_VALUE){
    ...
    if(obj && isObject(obj))
    {
        result = obj['taf_config_line_vector_value'];
        if(isArray(result))
        {
            ret = result;
        }
